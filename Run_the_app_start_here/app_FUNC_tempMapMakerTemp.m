function [] = app_FUNC_tempMapMakerTemp(app,TPind,plt_xt,ppath)
%------------------------------------------------------------------------
% The main function to generate temperature maps
...
%% Read the data and make the maps
    all_dir = app.TemperatureDir;
    all_data = app.TemperatureData;
    fileinp = [["PL_MAT_oceanmasked.txt"],["LGM_MAT_oceanmasked.txt"],...
    ...
    for i = 1:length(TPind)  % data = [lat      lon        t_PL        t_LGM       t_MH      t_PI      t_PD]
...
%         stat = [];
%     end
    functempmapmaker(app,fp,TP,LAT,LONG,mat_data,plt_xt,smpl,ice_loc,ppath);
end
%% make the map ready
function[] = functempmapmaker(app,fp,TP,LAT,LON,mat_data,plt_xt,smpl,ice_loc,ppath)
%------------------------------ set the figure
...
%     P_min = fix(nanmin(nanmin(nanmin(mat_data))));
    ...
    for i = 1:sz
        ...
        map_MAT(app,LAT,LON,mat_data(:,:,i),TP(i),lst,plt_xt,smpl,ice_loc,spacing);
...
%% plot the map
function map_MAT(app,LAT,LON,P,TP,lst,plt_xt,smpl,ice_loc,spacing)
    if app.CT_Fixed.Value == 1 % fix colorbar
        target_labels = [-100 -30 -25 -20 -15 -10 -7 -5 -3 -1 1 3 7 10 12 15 20 25 30 100];
...
%         clrs((clrs(:,1)==1 & clrs(:,2)==1 & clrs(:,3)==1),:) = [0.85 0.85 0.85];
...
%         colormap(clrs)
        if TP == lst    
            ...
            label = cb.Label;
            label.String = "Mean Annual Temperature [°c]";
            label.FontSize = 10;
        ...
        for ii = 1:length(target_labels)-1
            if ii == length(target_labels)-1
                P(P>=target_labels(ii)&P<=100000) = ii+100000;
            else
                P(P>=target_labels(ii)&P<target_labels(ii+1)) = ii+100000;
            end
        end 
        initial_labels = 1+100000:length(target_labels)+100000;
        surfm(LAT,LON,P)
...
function [] = funcpercmapsave(app,ppath,TP,f)
 ...
 %----------------------
    if app.CategorizedButton.Value == 1
        temppath = string(ppath)+filesep+"SavedFigures"+filesep+"TemperatureMaps";
    ...
    fn_map = sprintf("%sMeanAnnualTemperature_%s",map_path,tp);
    %---------------------------------
    if app.JPG.Value == 1
        i = 1;
        while true
