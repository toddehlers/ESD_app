...
% It extracts the data calculated as river erosion rate and cosmo erosion
% rate and generates a meaningful output of all the data next to each other
% The "BreifData" is one of the output of this function
...
%% find the values based on the indices
    ...
    %----------------------------------------- rows of OUT matrix:
    % 1: % response time
    % 2: % River erosion rate (RE)
    % 3: % change of RE relative to Ui
    % 4: % change of RE relative to PEAK_RE
    % 5: % Cosmogenic erosion rate (CE)
    % 6: % change of CE relative to Ui
    % 7: % change of CE relative to PEAK_CE
    % 8: % lateral shift in knick point position
    % 9: % the index
    %----------------------------------------- columns of OUT matrix:
    % 1:       @ peak of RE
    % 2:       @ peak of CE
    % 3:       @ model duration
    % 4:       @ end of the model
