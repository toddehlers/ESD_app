function [] = app_FUNC_MapMaker1(app,selected_data,plt_type,plt_at,plt_xt,ppath)
%------------------------------------------------------------------------
...
%------------------------------------------------------------------------
    ...
    for i = 1:size(selected_data,1)
        FUNC_map...
            (app,selected_data(i,:),plt_type,plt_at,plt_xt,LAT,LONG,P,ind_mesh,ice_loc,smpl,a3,ppath);
%         N_wb(end-1) = N_wb(end-1) +(N_wb(1)/size(selected_data,1));
    end
end
%%
function FUNC_map...
    (app,selected_data,plt_type,plt_at,plt_xt,LAT,LONG,P,ind_mesh,ice_loc,smpl,a3,ppath)
...
%----------------------------------------------------- run the map
            app.loadinglabel.Text = "Plotting... (" + str_plttyp + ")";
            pause(0.1);
...
%             app.ESDMMUIFigure.Visible = 'off';
%             app.ESDMMUIFigure.Visible = 'on';
...
%-----------------------------------------------
    folder_at = [   "General",...
                    "RiverErosionRelativeTo(Ui)",...
                    "RiverErosionRelativeTo(Peak)",...
                    "CosmoErosionRelativeTo(Ui)BeforePeak",...
                    "CosmoErosionRelativeTo(Ui)AfterPeak",...
                    "CosmoErosionRelativeTo(Peak)BeforePeak",...
                    "CosmoErosionRelativeTo(Peak)AfterPeak"    ];
    file_at = [ "PeakRE",...
                "PeakCE",...
                string(tt)+"[kyr]",...
                "End",...
...
%-----------------------------------------------
    if app.CategorizedButton.Value == 1
        temppath = string(ppath)+filesep+"SavedFigures"+filesep+"ErosionMaps"+filesep+TPi+"_"+TPn+filesep...
                         +num2str(L,'%.d')+"_"+num2str(Ui,'%.2f')+"_"+num2str(K,'%.0e')+filesep...
                         +str_plttyp+filesep+fld_nm;        
