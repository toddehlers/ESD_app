function [final_output,OUT,T,Tlong,RE,RElong,CE,CElong,KPT,KPTlong] = FUNC_ExtractParameters(FileInfo,PixelInfo,RiverData,CosmoData)
%% Read the data
% File info
    TPi = FileInfo(1);
    TPn = FileInfo(2);
    tt = str2num(char(FileInfo(3))); 
    L = str2num(char(FileInfo(4))); 
    K = str2num(char(FileInfo(5))); 
    Ui = str2num(char(FileInfo(6))); 
    Ndt = str2num(char(FileInfo(7))); 
% Pixel info
    Npixel = PixelInfo(1);
    lat = PixelInfo(2); 
    long = PixelInfo(3); 
    ...
    Pn = PixelInfo(6); 
    Tn = PixelInfo(7); 
% data from minus time to end (long data)
    Tlong = CosmoData(1,:);
    zro = find(Tlong == 0);
    befor_strt = abs(size(CosmoData,2) - size(RiverData,2));
