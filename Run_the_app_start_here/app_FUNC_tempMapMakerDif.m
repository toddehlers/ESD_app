...
%------------------------------------------------------------------------
    ...
    all_dir = app.TemperatureDir; % precipitations directory
    all_data = app.TemperatureData;  % precipitations data
    file = [["PL_MAT_oceanmasked.txt"],["LGM_MAT_oceanmasked.txt"],...
        ["MH_MAT_oceanmasked.txt"],["PI_MAT_oceanmasked.txt"],["PD_MAT_oceanmasked.txt"]]; 
...
%         stat = [];
%     end
    funcdifftempmapmaker(app,fp,TP,LAT,LONG,mat_diff,plt_xt,smpl,ice_loc,ppath);
end
function[] = funcdifftempmapmaker(app,fp,TP,LAT,LONG,mat_diff,plt_xt,smpl,ice_loc,ppath)
%----------------------------------------------- set the figure
...
function map_DIFF(app,LAT,LONG,D,TP,plt_xt,smpl,ice_loc,lst,spacing)
    if app.CT_Fixed.Value == 1 % fix colorbar
        target_labels = [-30 -20 -15 -10 -5 -2.5 -1 -0.5 0.5 1 2.5 5 10 15 20 30];
...
%         clrs((clrs(:,1)==1 & clrs(:,2)==1 & clrs(:,3)==1),:) = [];
        ...
        if TP == lst 
            ...
            label = cb.Label;
            label.String = "Mean Annual Temperature Difference [°c]";
            label.FontSize = 10;
        ...
        if neg > 0 && pos > 0
            target_labels_temp(target_labels_temp<=0.5 & target_labels_temp>=-0.5) = []; % delete 
            neg = length(find(target_labels_temp<0)); % number of negative values
            
            target_labels = zeros(1,length(target_labels_temp)+2);
            target_labels(1:neg) = target_labels_temp(1:neg);
            target_labels(neg+1) = -0.5;
            target_labels(neg+2) = 0.5;
        ...
        if neg == 0 % only use blue color
            clrs_bl = redblue(lcb*2-1);
            clrs_bl = clrs_bl(1:lcb-1,:);
            clrs = clrs_bl;
        elseif pos == 0 % only use red color
            clrs_rd = redblue(lcb*2-1);
            clrs_rd = clrs_rd(lcb+1:end,:);
            clrs = clrs_rd;
        else
            clrs_bl = redblue(pos*2-1);
            clrs_bl = flip(clrs_bl(1:pos-1,:),1);
            clrs_rd = redblue(neg*2-1);
            clrs_rd = flip(clrs_rd(neg+1:end,:),1);
            clrs = [clrs_rd;[0.85 0.85 0.85];clrs_bl];
        end
...
function [] = funcpercmapsave(app,ppath,TP,f)
 ...
 %----------------------
    if app.CategorizedButton.Value == 1
        temppath = string(ppath)+filesep+"SavedFigures"+filesep+"TemperatureDifferenceMaps";
...
%-----------------------------------------------
    ...
    fn_map = sprintf("%sMeanAnnualTemperatureDifference_%s",map_path,tp);
%-----------------------------------------------
    if app.JPG.Value == 1
        i = 1;
        while true
