...
% This function uses the following functions
    ...
    %1-app_FUNC_roundvalues
%------------------------------------------------------------------------
    %TPi, initial time period (string)
    % TPn, final time period (string)
    % L,Ui,K , river parameters (scalar)
    % x, length vector
    % tt, time duration between initial and final time period
    % BriefOutput, all the calculated data of the selected pixel
    % OUT, explained in app_FUNC_ExtractParameters
    % FinalTime, time vectore starts from -10000 to the end of the model
    % FinalRiver, River erosion rate vector same as FinalTime
    %       row1: river erosion rate, row2: % relative to Ui, row3: % relative to peak
    % FinalCosmo, Cosmogenic erosion rate vector same as FinalTime
    %       row1: cosmo erosion rate, row2: % relative to Ui, row3: % relative to peak
    % FinakKPT, Knickpoint shift vector same as FinalTime
    %       row1: knickpoint position, row2: amount of shift
    % Z, elevation change every 100 years at every x location
    % plt_type, the type of the profile
    
    % extract data from the selected row or pixel or lat,long from the BriefData
    Npixel = BriefOutput(1); % the selected pixel number
    lat = BriefOutput(2); % the pixels latitude
    long = BriefOutput(3); % the pixels longitude
    alt = BriefOutput(4); % the pixels altitude
    Pi = BriefOutput(5); % the pixels initial precipitattion rate
    Pn = BriefOutput(6); % the pixels final precipitation rate
    Tn = BriefOutput(7); % pixels time duration
    strt = find(FinalTime == -10000); % time index at time = -10000
    zro = find(FinalTime == 0); % time index at time = 0
%-------------------------------------------------------------- @ tt and Tn
    ...
    indG(:,tmp) = [];  %remove the columns with no data
%-------------------------- RE(UI) 10 20 30 40 50 %
% about river erosion rate relative to Ui
    ...
    tmp = find(isnan(indUI(1,:))); % find if there is a NaN
    indUI(:,tmp) = []; %remove the columns with no data
%-------------------------- RE(PK) 10 20 30 40 50 %
% about river erosion rate relative to Peak
...
%     tmp = find(isnan(indPK(1,:)));  % find if there is a NaN
%     indPK(:,tmp) = []; %remove the columns with no data
%--------------------------
% indALL(1st row) shows the index of all the points with data related to tt,Tn, and
% RE(Ui) which are sorted by time (Tn is always at the end)
% index @ RE(Ui) = 10 20 30 40 50 % tt Tn (sorted by time)
    indALL = [indG indUI]; 
    [~,tmp2] = sort(indALL(1,:));
    indALL = indALL(:,tmp2);
%--------------------------------------------------------------
    zALL = Z(indALL(1,:),:);
    kptALL = FinalKPT(1,indALL(1,:)+zro-1);
    for i = 1:size(indALL,2)
        kptALL(2,i) = zALL(end,(kptALL(1,i)/(L/1000))+1);
    end
%% plot    
% subplots positions                                                                       
...
%     y_rnd = app_FUNC_roundvalues(linspace(0,(max(max(Z))+max(max(Z))*0.01),8));
    y_rnd = round(linspace(0,(max(max(Z))+max(max(Z))*0.01),5),2);
...
%% Change in mean (river) erosion rate
    ...
    hold on
    [~,a2] = max(abs(REUi(zro:end)));
    plot(OUT(1,1),REUi(a2+zro-1),'db','MarkerFaceColor','blue','MarkerEdgeColor','blue','MarkerSize',8); % at max RE
    hold on 
    if Ui >= max(FinalRiver(1,strt:end))
        ytxt = -(abs( mn - mx )) * 0.03;
    else   
...
%% Change in mean (cosmo) erosion rate
    ...
    hold on
    [~,a2] = max(abs(CEUi(zro:end)));
    plot(OUT(1,2),CEUi(a2+zro-1),'dr','MarkerFaceColor','red','MarkerEdgeColor','red','MarkerSize',8); % at max CE
    hold on
    if Ui >= max(FinalRiver(1,strt:end))
        ytxt = -(abs( mn - mx )) * 0.03;
    else   
...
%% Chnage of knick points location 
    ...
    ymax = mx + (abs(mn-mx)*0.1);
    ylim([ymin ymax]);
%     y_rnd = app_FUNC_roundvalues(round(linspace(mn,mx,5),5));
%     yticks(y_rnd);
    yticks(round(linspace(mn,mx,5),5));
    %--------------------------------%
    yyaxis right
