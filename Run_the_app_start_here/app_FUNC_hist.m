function [] = app_FUNC_hist(app,ppath)
%------------------------------------------------------------------------
...
%------------------------------------------------------------------------
    ...
    for i = 1:size(selected_data,1)
        ...
        if app.hist1.Value == 1
            ...
            app.loadinglabel.Text = "Plotting... (response time histogram)";
            pause(0.1);
        ...
        if app.hist2.Value == 1
            ...
            app.loadinglabel.Text = "Plotting... (erosion histogram)";
            pause(0.1);
            [~,~,rtio] = app_FUNC_figureratio(app); % figure aspect ratio
            f = app_FUNC_setthefiguresize(app,rtio); % generate a figure
...
function hist_RT(app,RT,at,Nsmpl,Ui,L,K,TPi,TPn,fp)
    ...
    hist_mean = round(mean(r_mod_obs(:,3)),2);
    hist_med = round(median(r_mod_obs(:,3)),2);
    hist_std = round(std(r_mod_obs(:,3)),2);
    ...
    xlim([0 2])
    title('Calculated response time @ peak of cosmogenic erosion rate  / observed cosmogenic reosion rate response time','fontsize',14,'FontWeight','bold')
    ...
    'String',{'Number of pixels with cosmogenic sample points \rightarrow   '+string(st),...
                  ...
                  '--------------------',...
                  'Mean: '+string(hist_mean),...
                  'Median: '+string(hist_med),...
                  'Standard deviation: '+string(hist_std)},...
...
%     hist_std = round(std(h.BinEdges),2);
    ...
    hist_mean = round(mean(r_RE_CE(:,3)),2);
    hist_med = round(median(r_RE_CE(:,3)),2);
    hist_std = round(std(r_RE_CE(:,3)),2);
    ...
    plot([hist_med hist_med],[0 max(h.Values)+(0.05*max(h.Values))],'--c','linewidth',1)
    text(hist_med,max(h.Values)+(0.055*max(h.Values)),"Mdn.",'Color',[0 1 1],'fontsize',12)
...
%-----------------------------------------------
    if app.CategorizedButton.Value == 1
        temppath = string(ppath)+filesep+"SavedFigures"+filesep+"Histograms"+filesep+TPi+"_"+TPn+filesep...
