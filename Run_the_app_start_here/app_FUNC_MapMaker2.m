...
%------------------------------------------------------------------------
% The map maker function is responsible to generate maps.
% It is made by 3 functions as mapmaker1, mapmaker2
% Mapmaker 1 is responsibel to select data and some data modification and
% save the generated map at the end
% mapmaker2 is responsible to generate the maps, axis, colorbars and ...
...
% This function uses the following functions
    ...
    %2-app_FUNC_mapaxesmaker
    %3-app_FUNC_roundvalues
...
%% ---------------------------------------------------------- % main part of the map
    if ind_typ == 1
        [initial_labels,tlab] = FUNC_mapRT(app,P,LAT,LONG,Ui,at,tt,scb);
...
%% ---------------------------------------------------------- % Extra info on the map
   ...
   %---------------------------------------------------------- add ice LGM ice cover
    if plt_xt(4) == 1 % add LGM ice sheet location
        plotm(ice_loc(:,1),ice_loc(:,2),'+','color',[0.85 0.85 0.85],'MarkerSize',1.5) %plot LGM ice sheet
        txtall{end+1} = ['{\color[rgb]{0.25 0.25 0.25}+ \color[rgb]{0.25 0.25 0.25}LGM ice cover}'];
    end
    ...
    if plt_xt(3) == 1 % add cosmo sample points
        if ind_typ == 1 % two colors sample points for response time maps
            ...
            smpl1clr = [0 1 0.4]; % greenish
            smpl2 = find(smpl(:,3)>=smpl(:,7));
            smpl2 = smpl(smpl2,:); % where the samples have bigger value compare to the model values at that point
            smpl2clr = [1 0.4 0.2]; % redish
            if ~isempty(smpl1)
%                 plotm(smpl1(:,1),smpl1(:,2),'.','MarkerEdgeColor',[1 0.7 0],'MarkerSize',15)
                plotm(smpl1(:,1),smpl1(:,2),'.','MarkerEdgeColor',smpl1clr,'MarkerSize',15)
            end
            if ~isempty(smpl2)
%                 plotm(smpl2(:,1),smpl2(:,2),'.','MarkerEdgeColor','y','MarkerSize',15)
                plotm(smpl2(:,1),smpl2(:,2),'.','MarkerEdgeColor',smpl2clr,'MarkerSize',15)
            end
            txtall{end+1} = ['{\color[rgb]{0 1 0.4}\bullet \color[rgb]{0 1 0.4}observed cosmogenic response time < model response time}'];
            txtall{end+1} = ['{\color[rgb]{1 0.4 0.2}\bullet \color[rgb]{1 0.4 0.2}observed cosmogenic response time >= model response time}'];
        else % one color sample points for other type of maps
            plotm(smpl(:,1),smpl(:,2),'.','MarkerEdgeColor',[0 0.7 0],'MarkerSize',15);
            txtall{end+1} = ['{\color[rgb]{0 0.7 0}\bullet \color[rgb]{0 0.7 0}cosmo sample}'];
        end
    end
    ...
    txtall(1) = [];
%% ---------------------------------------------------------- % Strings on the map
    ttl_at = [  "peak of the river erosion rate",...
                "peak of the cosmogenic erosion rate",...
                string(round(tt,0))+" [kyr]",...
                "end of the model (500 [kyr] or earlier)",...
                "10% of river erosion rate relative to Ui",...
                "20% of river erosion rate relative to Ui",...
                "30% of river erosion rate relative to Ui",...
                "40% of river erosion rate relative to Ui",...
                "50% of river erosion rate relative to Ui",...
                "10% of the river erosion rate relative to the peak",...
                "20% of the river erosion rate relative to the peak",...
                "30% of the river erosion rate relative to the peak",...
                "40% of the river erosion rate relative to the peak",...
                "50% of the river erosion rate relative to the peak",...
                "10% of cosmogenic erosion rate relative to Ui before the peak",...
                "20% of cosmogenic erosion rate relative to Ui before the peak",...
                "30% of cosmogenic erosion rate relative to Ui before the peak",...
                "40% of cosmogenic erosion rate relative to Ui before the peak",...
                "50% of cosmogenic erosion rate relative to Ui before the peak",...
                "10% of cosmogenic erosion rate relative to Ui after the peak",...
                "20% of cosmogenic erosion rate relative to Ui after the peak",...
                "30% of cosmogenic erosion rate relative to Ui after the peak",...
                "40% of cosmogenic erosion rate relative to Ui after the peak",...
                "50% of cosmogenic erosion rate relative to Ui after the peak",...
                "10% of cosmogenic erosion rate relative to the peak before the peak",...
                "20% of cosmogenic erosion rate relative to the peak before the peak",...
                "30% of cosmogenic erosion rate relative to the peak before the peak",...
                "40% of cosmogenic erosion rate relative to the peak before the peak",...
                "50% of cosmogenic erosion rate relative to the peak before the peak",...
                "10% of cosmogenic erosion rate relative to the peak after the peak",...
                "20% of cosmogenic erosion rate relative to the peak after the peak",...
                "30% of cosmogenic erosion rate relative to the peak after the peak",...
                "40% of cosmogenic erosion rate relative to the peak after the peak",...
...
%% ---------------------------------------------------------- RT
function [initial_labels,tlab] = FUNC_mapRT(app,P,LAT,LONG,Ui,at,tt,scb)
...
%         spacing = app_FUNC_colobarspacing(P,0.1,Pmin,tt,10); % make spacing for colorbar
        ...
        if app.CT_Fixed.Value == 1 %fixed response time colorbar (always till 500ky)
            ...
            spacing = app_FUNC_TargetLabel(app,P);
...
function [initial_labels,tlab] = FUNC_mapRECE_UiPK(P,LAT,LONG,Ui,scb)
    ...
    la = length(A);
    TwoColro = false;
%     atemp = [10 20 30 40 50];
%     if length(A) == 2 || length(A) == 4 || length(A) == 6
%         A1 = A(1:la/2);
%         A2 = flip(A((la/2)+1:end));
%         if sum(A1==-A2) == la/2
%             TwoColro = true;
%             for aa = 1:length(atemp)
%                 if abs(mean(A2) - atemp(aa)) < 2
%                     AA = [-atemp(aa) atemp(aa)];
%                     break
%                 end
%             end    
%         end
%     end
    if TwoColro == true % only the case with two values (positive and negative of a same value)
        target_labels = [AA];
        for aa = 1:la/2
            P(P==A(aa)) = 10002;
            P(P==A(aa+la/2)) = 10004;
